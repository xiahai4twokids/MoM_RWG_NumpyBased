...
class K_fun(object):
    def __init__(self, greenFunc,r1Group_12,r2Group_12):
        self.kernelGreen = greenFunc
        self.k = greenFunc.k
        self.r1Group_12 = r1Group_12
        self.r2Group_12 = r2Group_12        
        self.shape = [r1Group_12.shape[0],r2Group_12.shape[0]]        
        pass
    
    def getRow(self,k):
        r_1 = np.zeros([3,1,self.r2Group_12.shape[0]])
        r_2 = np.zeros([3,1,self.r2Group_12.shape[0]])
        for id_comp in xrange(3):
            r_2[id_comp,:,:],r_1[id_comp,:,:] = np.meshgrid(self.r2Group_12[:,id_comp], self.r1Group_12[k-1:k,id_comp])
        K12 = self.kernelGreen._ejkr_r(r_1,r_2)
        assert(np.sum(np.isnan(K12))==0) 
        return K12
    
    def getCol(self,k):
        r_1 = np.zeros([3,self.r1Group_12.shape[0],1])
        r_2 = np.zeros([3,self.r1Group_12.shape[0],1])
        for id_comp in xrange(3):
            r_2[id_comp,:,:],r_1[id_comp,:,:] = np.meshgrid(self.r2Group_12[k-1:k,id_comp], self.r1Group_12[:,id_comp])
        K12 = self.kernelGreen._ejkr_r(r_1,r_2)
        assert(np.sum(np.isnan(K12))==0) 
        return K12
    ...
    def fillingProcess_kernel(self,var):
        try:        
            ...
            if ii_index!=jj_index and gridinDomain[ii_index]&gridinDomain[jj_index]: # 两分区相近
                ...
                return tempZ                
    #            with open('tempData/fillingProcess_impMatrix_%d_%d.dat'%(ii_index,jj_index),'wb') as f:
    #                pickle.dump(tempZ,f)
                pass
...
from _lu_utils import RankRevealingMethod_tradition
...
#                return tempZ
    ...
    #            pass
                ...
                r1,r2 = tempZ[0]
                K12 = K_fun(GreenFunc(self.k),r1,r2)
                rows = K12.shape[0]
                columns = K12.shape[1]
                result = RankRevealingMethod_tradition().LUDec(matrix=K12, rows= rows, columns=columns, \
                                                          threshold_remain = SolverPar().aca_threshold, \
                                                          threshold_rank=-1)
                result2 = [(result[0],result[1]),tempZ[1],tempZ[2],tempZ[3]]
                return result2
#                with open('tempData/fillingProcess_aca_impMatrix_%d_%d.dat'%(ii_index,jj_index),'wb') as f:
